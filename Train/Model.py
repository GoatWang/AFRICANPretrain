...
class VideoFrameIdenetity(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.train_metrics.update(torch.argmax(torch.softmax(frame_logits), dim=1), torch.argmax(ground_truth, dim=1))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        ground_truth = labels_onehot.to(frame_logits.device).view(-1, labels_onehot.shape[-1])
        loss = self.loss_func(frame_logits, ground_truth.type(torch.float32))
        self.valid_metrics.update(torch.argmax(torch.softmax(frame_logits), dim=1), torch.argmax(ground_truth, dim=1))
